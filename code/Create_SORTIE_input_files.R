...
combined_names <-  apply(expand.grid(stands$ID, substr(names(thinning_regimes),10,14)), 1, paste, collapse="_")
...
## Climate CCLM_45
 xml_files <- pmap(list(narrative, "CCLM_45"), parse_xml)

  map(seq_along(xml_files), function(i){
    write_xml(xml_files[[i]],
              file=paste0("D:/Activity/Thinning_CC/Thinning_Cat/data/SORTIE_Inputs/", names(xml_files)[[i]],"_CCLM_45.xml")) })
